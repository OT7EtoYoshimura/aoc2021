...
both_tco([_T], CountP1, CountP2)	->	io:format("P1: ~p~nP2: ~p~n", [CountP1, CountP2]);
both_tco([_H1|T] = [H1|[H2|[_H3|[H4|_T2]]]], CountP1, CountP2) ->
	both_tco(T,
			 if H1 < H2 -> CountP1+1; true -> CountP1 end,
			 if H1 < H4 -> CountP2+1; true -> CountP2 end);
both_tco([_H1|T] = [H1|[H2|_T2]], CountP1, CountP2) when H1 < H2	-> both_tco(T, CountP1+1, CountP2);
both_tco([_H1|T], CountP1, CountP2)									-> both_tco(T, CountP1, CountP2).

p1_tco([_T], Count)									-> io:format("P1: ~p~n", [Count]);
p1_tco([_H1|T] = [H1|[H2|_T2]], Count) when H1 < H2	-> p1_tco(T, Count+1);
p1_tco([_H1|T], Count)								-> p1_tco(T, Count).

p2_tco(List, Count) when length(List) < 4 						-> io:format("P2: ~p~n", [Count]);
p2_tco([_H1|T] = [H1|[_H2|[_H3|[H4|_T2]]]], Count) when H1 < H4	-> p2_tco(T, Count+1);
p2_tco([_H1|T], Count)											-> p2_tco(T, Count).
